import { SourcingProductPage } from "@/components/SourcingProductPage";
import { buildWebsiteMetadata } from "@/lib/seo";
import { sourcingProducts } from "@/lib/sourcingProducts";

const product = sourcingProducts["floor-washers"];

export const metadata = buildWebsiteMetadata({ title: "Hard Floor Washer Manufacturers & Sourcing in China", description: product.metaDescription, canonical: "/sourcing/floor-washers", image: product.image, robots: { index: true, follow: true } });

export default function Page() {
  return <SourcingProductPage product={product} />;
}

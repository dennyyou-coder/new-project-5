...
test("commercial-cleaning batch fifteen publishes Cleanfix, Comac and Fimap without article relationships", async () => {
  const newSlugs = ["cleanfix", "comac", "fimap"];
  const profilesBySlug = new Map(
    getBrandProfiles().map((candidate) => [candidate.slug, candidate])
  );
  const realArticles = getInsights();
...
test("commercial-cleaning batch twelve publishes Tennant, Hako and TASKI without article relationships", async () => {
  const newSlugs = ["hako", "taski", "tennant"];
  const profilesBySlug = new Map(
    getBrandProfiles().map((candidate) => [candidate.slug, candidate])
  );
  const realArticles = getInsights();
  ...
  for (const slug of newSlugs) {
    ...
    const memberships = categories
      .filter(({ profiles }) => profiles.some((profile) => profile.slug === slug))
      .map(({ category }) => category.slug);
    assert.deepEqual(memberships, ["commercial-industrial-cleaning"]);
    assert.equal(getBrandCategoryForProfile(slug)?.slug, "commercial-industrial-cleaning");
  }

  assert.equal(profilesBySlug.get("tennant").schemaEntityType, "Organization");
  assert.equal(profilesBySlug.get("hako").schemaEntityType, "Organization");
  assert.equal(profilesBySlug.get("taski").schemaEntityType, "Brand");
  assert.match(profilesBySlug.get("taski").legalEntityNote, /Solenis|Diversey/i);
});
...
test("outdoor-equipment batch ten profiles publish with verified brand and operating boundaries", async () => {
  const newSlugs = ["ego-power-plus", "gardena", "toro"];
  const profilesBySlug = new Map(
    getBrandProfiles().map((candidate) => [candidate.slug, candidate])
  );
  const realArticles = getInsights();
  ...
  for (const slug of newSlugs) {
    ...
    assert.deepEqual(taggedArticles, [], `${slug} must not create article relationships`);
    assert.deepEqual(membershipsFor(slug), ["lawn-garden-equipment"]);
    assert.equal(getBrandCategoryForProfile(slug)?.slug, "lawn-garden-equipment");
  }

  assert.match(profilesBySlug.get("ego-power-plus").ownership.summary, /Chervon/i);
  assert.match(profilesBySlug.get("ego-power-plus").legalEntityNote, /brand|regional|warranty/i);
  assert.match(profilesBySlug.get("gardena").ownership.summary, /Husqvarna/i);
  assert.match(profilesBySlug.get("gardena").legalEntityNote, /brand|division|regional/i);
  assert.match(profilesBySlug.get("toro").ownership.summary, /The Toro Company/i);
  assert.match(profilesBySlug.get("toro").legalEntityNote, /brand|company|regional/i);
});
...
test("home-appliance batch eight profiles publish with current ownership and operating boundaries", async () => {
  ...
  assert.match(profilesBySlug.get("asko").ownership.summary, /Hisense/i);
  assert.match(profilesBySlug.get("asko").legalEntityNote, /Hisense Europe|Gorenje/i);
  assert.match(profilesBySlug.get("teka").ownership.summary, /Midea/i);
  assert.match(profilesBySlug.get("teka").legalEntityNote, /Teka Rus|excluding/i);
  assert.match(
    profilesBySlug.get("hamilton-beach").ownership.summary,
    /Hamilton Beach Brands Holding Company/i
  );
  assert.match(
    profilesBySlug.get("hamilton-beach").legalEntityNote,
    /Hamilton Beach Brands, Inc.|operating/i
  );
});
...
test("home-appliance batch seven profiles publish with regional brand and operating boundaries", async () => {
  ...
  assert.match(profilesBySlug.get("xiaomi-mijia").legalEntityNote, /ecosystem|manufacturer/i);
  assert.match(profilesBySlug.get("breville-sage").ownership.summary, /Breville Group Limited|BRG Group/i);
  assert.match(profilesBySlug.get("breville-sage").legalEntityNote, /United Kingdom|Europe|regional/i);
  assert.match(profilesBySlug.get("braun-household").ownership.summary, /De.?Longhi.*perpetual licen[cs]e/i);
  assert.match(profilesBySlug.get("braun-household").legalEntityNote, /P&G|Procter.*Gamble/i);
  assert.match(profilesBySlug.get("braun-household").legalEntityNote, /personal care/i);

  assert.deepEqual(membershipsFor("xiaomi-mijia"), [
    "floorcare-home-cleaning",
    "home-appliances-small-appliances"
  ]);
  assert.deepEqual(membershipsFor("breville-sage"), [
    "home-appliances-small-appliances"
  ]);
  assert.deepEqual(membershipsFor("braun-household"), [
    "home-appliances-small-appliances"
  ]);
  assert.equal(getBrandCategoryForProfile("xiaomi-mijia")?.slug, "floorcare-home-cleaning");
  assert.equal(
    getBrandCategoryForProfile("breville-sage")?.slug,
    "home-appliances-small-appliances"
  );
  assert.equal(
    getBrandCategoryForProfile("braun-household")?.slug,
    "home-appliances-small-appliances"
  );
});
...
test("home-appliance batch six profiles publish independently with verified entity boundaries", async () => {
  ...
  assert.match(profilesBySlug.get("rowenta").legalEntityNote, /Rowenta France|SEB Développement/i);
  assert.match(profilesBySlug.get("rowenta").legalEntityNote, /trademark/i);
  assert.match(profilesBySlug.get("gorenje").ownership.summary, /Hisense/i);
  assert.match(profilesBySlug.get("gorenje").legalEntityNote, /Gorenje gospodinjski aparati|Hisense Europe/i);
  assert.match(profilesBySlug.get("gorenje").legalEntityNote, /trademark/i);
  assert.match(profilesBySlug.get("supor").ownership.summary, /SEB INTERNATIONALE/i);
  assert.match(profilesBySlug.get("supor").legalEntityNote, /Zhejiang Supor/i);
  assert.match(profilesBySlug.get("supor").legalEntityNote, /trademark/i);
  assert.deepEqual(profilesBySlug.get("rowenta").aliases, ["ROWENTA"]);
  ...
  assert.deepEqual(membershipsFor("gorenje"), [
    "floorcare-home-cleaning",
    "home-appliances-small-appliances"
  ]);
  assert.deepEqual(membershipsFor("supor"), [
    "floorcare-home-cleaning",
    "home-appliances-small-appliances"
  ]);
  assert.equal(getBrandCategoryForProfile("rowenta")?.slug, "floorcare-home-cleaning");
  assert.equal(
    getBrandCategoryForProfile("gorenje")?.slug,
    "home-appliances-small-appliances"
  );
  assert.equal(
    getBrandCategoryForProfile("supor")?.slug,
    "home-appliances-small-appliances"
  );
});

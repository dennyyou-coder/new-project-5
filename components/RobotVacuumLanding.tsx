...
export function RobotVacuumLanding({ product }: { product: SourcingProduct }) {
  ...
  return (
    <div className="sourcing-v3-page sourcing-lawn-page sourcing-robot-vacuum-page">
      ...
      <section className="section section-muted sourcing-lawn-faq-section"><div className="sourcing-v3-container sourcing-lawn-faq"><div><p className="sourcing-v3-kicker">Industry Opportunity Questions</p><h2>The Product Decisions Buyers Are Debating Now</h2><p>These commercial and technical choices shape where robot vacuum growth can come from next.</p><img className="sourcing-lawn-faq-visual" src={directions[5]?.image || product.image} alt={directions[5]?.imageAlt || product.imageAlt} loading="lazy" decoding="async" /></div><div>{faqs.map(([question, answer]) => <details key={question}><summary>{question}</summary><p>{answer}</p></details>)}</div></div></section>
